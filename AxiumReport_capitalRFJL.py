...
class Reporting:
    ...
    #Removal of Disposal off the main JBdf cd 
    ...
    transferdf = transferdf[(transferdf["Supplier"].isna()) & (transferdf.Source == "Asset Assign Accounting")]
  ...
  #Additiondf utlizing the not (bitwise operator)
   # Additiondf = jobcostdf[((~jobcostdf['Line Memo'].str.contains('Tsfr', na = False)) | (~jobcostdf['Journal Memo'].str.contains('Tsfr', na = False))) & ((jobcostdf.Source != 'Asset Assign Accounting') & (jobcostdf['Worktags'].str.contains("Fund", na =False)))]
    ...
    Additiondf = Additiondf[ ((Additiondf["Source"] == 'Asset Assign Accounting') & (Additiondf["Supplier"].notna()) |  Additiondf['Worktags'].str.contains('fund') | Additiondf['Worktags'].str.contains('Fund')) | ~Additiondf['Source'].str.contains("Asset Assign Accounting")]

    #Creating Stats
    Total_Jb = sum(jobcostdf['Amount'])
    Total_Add = sum(Additiondf['Amount'])
    Total_trans = sum(transferdf['Amount'])
    Total_dis = sum(disposaldf['Amount'])
